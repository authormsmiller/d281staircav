---
slug: chieu-hoi-fsb-fontaine-1971-05
title: Chieu Hoi Rallier — Highway 331 Near FSB Fontaine
status: published
type: incident
date: 1971-05
date_known: false
date_note: |
  Date confirmed as pre-August 4, 1971 by the presence of Dennis Alloway as RTO in the photographs. Month (May) is approximate based on operational sequence. Day unknown.
location: Highway 331, near FSB Fontaine, Long Khanh Province, RVN
platoons:
  - name: Cat Platoon
    role: Escorted the rallier from Highway 331 to FSB Fontaine
contains:
  - slug: alloway-dennis
    name: Dennis Alloway
    role: Cat Platoon RTO
    note: |
      Presence in photographs confirms pre-August 4, 1971 date for this event. Identified by contemporary handwritten caption. Deceased.
  - slug: small-bill
    name: Bill Small
    role: Cat Platoon
    note: Identified by contemporary handwritten caption on Miller photograph
  - slug: cate-larry
    name: Larry Cate
    role: Cat Platoon
    note: Identified by contemporary handwritten caption. Deceased 2011.
  - slug: sells-leroy
    name: Leroy Sells
    role: Cat Platoon
    note: Identified by contemporary handwritten caption on Miller photograph
tagged:
  - slug: miller-marvin-dale
    name: SGT Marvin D. Miller
    note: |
      Photographer; likely present as escort or observer. Not named in the caption of his own photographs, but presence is probable given his Cat Platoon association and the photograph sequence.
  - davis-kirk
  - hilts-doug
  - weaver-ken
images:
  - filename: HLMVietnam1971_0147positiveEdit.jpg
    caption: |
      Chieu Hoi rallier escorted by Cat Platoon along Highway 331 toward FSB Fontaine. Contemporary handwritten caption names Alloway, Small, Cate, and Sells. Intelligence officer in clean uniform visible; unidentified.
    credit: Photographed by SGT Marvin D. Miller
    credit_slug: miller-marvin-dale
    date_approx: 1971-05
    date_known: false
    contains:
      - alloway-dennis
      - small-bill
      - cate-larry
      - sells-leroy
records:
  - type: physical-artifact
    title: Woodblock propaganda stamp
    holder: Miller family
    note: |
      Wooden stamp used to print VC propaganda; taken as a souvenir by Marvin Miller. Has never been inked. Printing it would recover the actual propaganda impression. Recommended for specialist identification: Texas Tech Vietnam Center and Archive.
  - type: physical-artifact
    title: Black fabric
    holder: Miller family
    note: |
      Black fabric associated with the rallier or the escort; exact provenance unclear. Held in Miller family collection.
related_events:
  - slug: contact-fsb-fontaine-1971-04-20
    relationship: operational-period
    note: |
      Both events occurred at or near FSB Fontaine in the April-May 1971 operational period. No causal relationship — loose operational linking only.
open_questions:
  - id: oq-01
    publish: true
    question: |
      An intelligence officer in a clean uniform appears in the photographs but has not been identified. If you recognize him or were present during this escort, please contact us.
  - id: oq-02
    publish: true
    question: |
      The woodblock propaganda stamp taken from the rallier has never been inked. We believe printing it would recover the original propaganda impression. If you have expertise in Vietnamese propaganda materials from this period, or can connect us with a specialist, we want to hear from you.
  - id: oq-03
    publish: false
    question: |
      Confirm with Bill Small, Leroy Sells, or any surviving Cat Platoon member: can they provide additional detail — the rallier's demeanor, what happened to him after he was brought in, whether there was a formal interrogation?
  - id: oq-04
    publish: false
    question: |
      The intelligence officer is in a clean uniform suggesting a rear-echelon unit. Check unit records for any attached intelligence personnel at FSB Fontaine in the April-May 1971 period.
archivist_notes:
  created: 2026-05-11T00:00:00.000Z
  session: 18
  sources:
    - Miller photograph collection (HLMVietnam1971_0147positiveEdit.jpg)
    - Contemporary handwritten caption — primary source for location and soldier identification; supersedes oral accounts placing the event at FSB Fanning
    - MDM-ANECDOTE-PRISONER (project file)
  location_note: |
    Earlier oral accounts suggested FSB Fanning. The contemporary handwritten caption states Highway 331, FSB Fontaine. The caption is treated as authoritative for location.
  provenance_note: |
    SGT Marvin D. Miller is the father of the archivist, Michael Miller. The photographs are in the Miller family collection.
---

## Summary

In approximately May 1971, a Viet Cong rallier surrendered voluntarily under the Chieu Hoi amnesty program, approaching FSB Fontaine along Highway 331. Cat Platoon escorted him in. The event was photographed by SGT Marvin D. Miller. Soldiers named in Miller's contemporary handwritten caption are Dennis Alloway (RTO), Bill Small, Larry Cate, and Leroy Sells — all Cat Platoon.

An intelligence officer in a clean uniform appears in the photographs but has not been identified. Two physical artifacts from this event — a woodblock propaganda stamp and a piece of black fabric — remain in the Miller family's possession.

## Context and Contested Details

**The Chieu Hoi program:** Chieu Hoi ("Open Arms") was a South Vietnamese government amnesty program offering VC and NVA combatants the opportunity to surrender voluntarily. Ralliers were not treated as prisoners of war. The man escorted by Cat Platoon walked in voluntarily.

**Location source:** An earlier oral account placed this event at FSB Fanning. The contemporary handwritten caption on the Miller photograph states Highway 331, FSB Fontaine. The photograph caption supersedes later recollection. Fontaine is used throughout this record.

**Date precision:** Alloway's presence establishes an upper bound — he died before August 4, 1971. The month (May) is approximate based on operational sequence in McGrew's calendar. The day is unknown.

**Woodblock stamp:** The stamp has never been inked since coming into the family's possession. Printing it would recover the actual propaganda impression — potentially a unique document of what the VC were distributing in Long Khanh Province in 1971.
